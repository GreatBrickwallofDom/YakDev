...
while [[ $internet == "unreachable" ]]; do
  ...
  if [[ $? -ne 0 ]]; then
    echo "Ext VPN address not reachable, waiting for cell link. Trying again!"
  else
    internet="reachable"
  fi
done
...
echo "Initial VPN connection attempt..."
...
while [[ true ]]; do
  ...
  if [[ $? -ne 0 ]]; then   #if ping is nonzero...
    echo "ping failed trying connection"
